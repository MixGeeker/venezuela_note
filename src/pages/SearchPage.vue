    ...
    <template v-else>
      <div class="mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900 mt-2">
          <template v-if="searchTerm">Search results for "{{ searchTerm }}"</template>
          <template v-else>Search</template>
        </h2>
        <p v-if="searchTerm && notesStore.searchResults.length > 0" class="text-sm text-gray-500 mt-1">
          Found {{ notesStore.searchResults.length }} result{{ notesStore.searchResults.length === 1 ? '' : 's' }}
        </p>
      </div>

      <div v-if="searchTerm && notesStore.searchResults.length === 0" class="text-center py-12 text-gray-400">
      ...
      <div v-else class="grid gap-3">
        ...
        >
          <h3 class="font-medium text-gray-900">{{ result.title || result.name.replace(/\.md$/, '') }}</h3>
...
const searchTerm = computed(() => tag.value ? `#${tag.value}` : query.value)
